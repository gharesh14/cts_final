...
async function loadAppealedRequests() {
    try {
        const response = await fetch('/appealed-requests');
        if (!response.ok) {
            throw new Error('Failed to fetch appealed requests');
        }
        const data = await response.json();
        updateAppealedRequestsStats(data.appealed_requests || []);
    } catch (error) {
    ...
    row.innerHTML = `
        ...
        <td>
            <div class="action-buttons">
                <button class="btn btn-primary btn-sm" onclick="runMLPrediction('${appeal.item_id}')">
                    <i class="fas fa-brain"></i> Re-run ML
                </button>
                <button class="btn btn-secondary btn-sm" onclick="viewAppealDetails('${appeal.item_id}')">
